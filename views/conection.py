...
@connection_bp.route('/delete_connection/<int:id_connection>', methods=['POST'])
def delete_connection(id_connection):
    if request.method == 'POST':
        try:
            api_url = f"{API_URL}/connections/{id_connection}"
...
@connection_bp.route('/edit_connection/<int:id_connection>', methods=['POST'])
def edit_connection(id_connection):
    if request.method == 'POST':
        try:
            ds_name_connection  = request.form.get('ds_name_connection')
            ds_user             = request.form.get('ds_user')
            ...
            api_url = f"{API_URL}/connections/{id_connection}"
            response = requests.put(api_url, json=connection_data)
